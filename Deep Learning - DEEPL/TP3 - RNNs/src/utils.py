...
class RNN(nn.Module):
    def __init__(self, nx, nh, ny):
        super().__init__()
        self.Wxh = nn.Parameter(torch.randn(nx, nh) * 0.1)
        self.Whh = nn.Parameter(torch.randn(nh, nh) * 0.1)
        self.bh  = nn.Parameter(torch.zeros(nh))

        self.Why = nn.Parameter(torch.randn(nh, ny) * 0.1)
        self.by  = nn.Parameter(torch.zeros(ny))

        self.nh = nh

    def geth0(self, B):
        return torch.zeros(B, self.nh, device=device)

    def one_step(self, x, h_prev):
        h = x @ self.Wxh + h_prev @ self.Whh + self.bh
        return torch.tanh(h)
